<?php
	...
	header("Content-Disposition: attachment; filename=$date-laporan-penjualan.xls");
    include "../koneksi.php";
?>
...
<table class="table mt-4" border="1">
    ...
    if (isset($_POST['date'])) {
        ...
        INNER JOIN pelayans
        on pelayans.id = pesanans.pelayan_id WHERE pesanans.tanggal BETWEEN '$from' AND '$to'" ;
    } else {
        $sql = "SELECT pesanans.id, pesanans.nama_pelanggan, pesanans.table_id, pesanans.bayar, pesanans.tanggal, pesanans.total, tables.table_nama, pelayans.pelayan_nama
        FROM  tables
        INNER JOIN pesanans
